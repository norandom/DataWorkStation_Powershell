...
4. Do not start any capture without explicit operator authorization. If capture is approved, prefer `profile-native-record <name> -Seconds <bounded-duration>`. Use start/stop only when an interactive reproduction requires it; always ensure stop or cancel runs.
