...
The separate default `RootlessPodman` module creates a clean named `Debian-MW` WSL 2 distribution, bootstraps pinned pyinfra locally, and applies `linux/rootless_podman.py`. It uses Debian's Podman package as a local, rootless, daemonless engine with user-scoped overlay storage and seccomp. The Podman API socket and services remain disabled. `.wsl-env` keeps the developer and malware distro/user selectors separate. Test with `./Apply-Workstation.ps1 -Mode Test -Module RootlessPodman`; repair with `-Mode Ensure`. This module never exports, imports, or clones the developer distro.
