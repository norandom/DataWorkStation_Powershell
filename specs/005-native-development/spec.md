...
### User Story 1 - Compile from every ordinary PowerShell session (Priority: P1)

A developer opens either supported PowerShell runtime and uses the native x64 compiler, linker, and
build engine directly without first finding or launching a special developer prompt.

**Why this priority**: Always-available native compilation is the primary requested outcome and is
required by Rust and native project builds.

**Independent Test**: Start clean Windows PowerShell 5.1 and PowerShell Core processes, load the
managed profile, inspect the declared environment, then compile and execute minimal C and C++
programs.

**Acceptance Scenarios**:

1. **Given** a new managed PowerShell session, **When** the developer resolves the native compiler, linker, and build engine, **Then** each command is immediately available for the declared x64 host and target.
2. **Given** a new managed shell, **When** a simple C or C++ source is compiled and linked, **Then** the resulting program executes successfully.
3. **Given** both supported PowerShell runtimes, **When** their public native-development environment is compared, **Then** they expose the same toolchain selection and compiler variables.
...
- REQ-007: When a managed PowerShell session starts, the profile shall dynamically select the current compatible build-tools instance for an x64 host and x64 target.
...
- REQ-009: When a conflicting non-Microsoft linker command exists earlier in the inherited path, the profile shall resolve the Microsoft linker for the managed native-development session.
- REQ-010: The managed compiler state shall declare persistent user values for `CC=cl.exe` and `CXX=cl.exe`.
...
- REQ-012: When either supported PowerShell runtime loads the managed profile, it shall expose equivalent native-development commands and declared environment values.
...
- REQ-025: The native-development profile initialization shall be idempotent within one shell without duplicating inherited path entries.
...
- **SC-001**: One hundred percent of new managed PowerShell 5.1 and PowerShell Core sessions resolve all thirteen declared development commands without a separate developer prompt.
- **SC-002**: Minimal C, C++, CMake, direct build-engine, Rust, package-test, and Java fixtures each complete successfully from a fresh managed shell.
...
- **SC-005**: Both supported PowerShell runtimes report identical compiler-selection, generator, Rust-directory, native-host, and Java-home values.
- **SC-006**: A second compliant repair changes zero managed toolchain resources, and repeated profile initialization produces no duplicate managed path entry.
